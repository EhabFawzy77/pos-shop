...
async function showInvoiceDetail(id) {
  ...
  try {
    ...
    table.innerHTML = sale.items
      .map(
        (item, index) => `
      <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 fade-in" style="animation-delay: ${
        index * 0.1
      }s;">
        <td class="p-3 border border-gray-300 dark:border-gray-600">${
          item.item
        ...
        <td class="p-3 border border-gray-300 dark:border-gray-600 font-bold text-green-600 dark:text-green-400">${
          item.total
        } جنيه</td>
      </tr>
    `
...
async function saveSale() {
  ...
  const data = {
    ...
    total: currentSale.reduce((sum, s) => sum + s.total, 0),
  };
  try {
    ...
    loadDailyIncome();
    loadWeeklyIncome();
    document.getElementById("saveSale").classList.add("bounce");
    setTimeout(
      () => document.getElementById("saveSale").classList.remove("bounce"),
      600
    );
    confettiBurst();
    showToast();
...
async function loadDailyIncome() {
  try {
    ...
    document.getElementById("dailyIncome").textContent = data.totalIncome;
  } catch (err) {
    console.error(err);
  }
}
...
async function loadWeeklyIncome() {
  try {
    ...
    const data = await res.json();
    document.getElementById("weeklyIncome").textContent = data.totalIncome;
  } catch (err) {
    console.error(err);
  }
}
